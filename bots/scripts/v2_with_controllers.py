...
class V2WithControllers(StrategyV2Base):
    ...
    def custom_command(
        ...
        """
        self.logger().info("api strategy 1")
        self.logger().info(
        ...
        for controller_id, controller in self.controllers.items():
            if hasattr(controller, "custom_command"):
                self.logger().info("api strategy 2")
                try:
                    self.logger().info(
                        f"[Manual] Forwarding custom command to controller={controller_id},"
                        f"custom_command={custom_command}, params={params}"
                    )
                    ...
                    if executor_ids:
                        ...
                        for executor_id in executor_ids:
                            ...
                            if executor:
                                self.logger().info("api strategy 3")
                                try:
                                    self.logger().info(
