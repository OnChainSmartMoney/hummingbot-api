...
class PMMisterConfig(ControllerConfigBase):
    ...
    portfolio_allocation: Decimal = Field(default=Decimal(
        "0.05"), json_schema_extra={"is_updatable": True})
    target_base_pct: Decimal = Field(default=Decimal(
        "0.2"), json_schema_extra={"is_updatable": True})
    min_base_pct: Decimal = Field(default=Decimal(
        "0.1"), json_schema_extra={"is_updatable": True})
    max_base_pct: Decimal = Field(default=Decimal(
        "0.4"), json_schema_extra={"is_updatable": True})
    buy_spreads: List[float] = Field(
        default="0.01,0.02", json_schema_extra={"is_updatable": True})
    sell_spreads: List[float] = Field(
        default="0.01,0.02", json_schema_extra={"is_updatable": True})
    buy_amounts_pct: Union[List[Decimal], None] = Field(
        default="1,2", json_schema_extra={"is_updatable": True})
    ...
    buy_cooldown_time: int = Field(
        default=15, json_schema_extra={"is_updatable": True})
    sell_cooldown_time: int = Field(
        default=15, json_schema_extra={"is_updatable": True})
    buy_position_effectivization_time: int = Field(
        default=60, json_schema_extra={"is_updatable": True})
    sell_position_effectivization_time: int = Field(
        default=60, json_schema_extra={"is_updatable": True})

    # Price distance requirements
    min_buy_price_distance_pct: Decimal = Field(default=Decimal(
        "0.003"), json_schema_extra={"is_updatable": True})
    min_sell_price_distance_pct: Decimal = Field(
        default=Decimal("0.003"), json_schema_extra={"is_updatable": True})

    leverage: int = Field(default=20, json_schema_extra={"is_updatable": True})
    position_mode: PositionMode = Field(default="HEDGE")
    take_profit: Optional[Decimal] = Field(default=Decimal(
        "0.0001"), gt=0, json_schema_extra={"is_updatable": True})
    open_order_type: Optional[OrderType] = Field(
        default="LIMIT_MAKER", json_schema_extra={"is_updatable": True})
    take_profit_order_type: Optional[OrderType] = Field(
        default="LIMIT_MAKER", json_schema_extra={"is_updatable": True})
    max_active_executors_by_level: Optional[int] = Field(
        default=4, json_schema_extra={"is_updatable": True})
    tick_mode: bool = Field(default=False, json_schema_extra={
                            "is_updatable": True})
    ...
    def update_parameters(self, trade_type: TradeType, new_spreads: Union[List[float], str],
        ...
        if new_amounts_pct is not None:
            ...
            setattr(self, amounts_pct_field, [
                    1 for _ in getattr(self, spreads_field)])
    ...
    def get_spreads_and_amounts_in_quote(self, trade_type: TradeType) -> Tuple[List[float], List[float]]:
        ...
        if trade_type == TradeType.BUY:
            normalized_amounts_pct = [
                amt_pct / total_pct for amt_pct in buy_amounts_pct]
        else:
            normalized_amounts_pct = [
                amt_pct / total_pct for amt_pct in sell_amounts_pct]
...
class PMMister(ControllerBase):
    ...
    def __init__(self, config: PMMisterConfig, *args, **kwargs):
        ...
        self.market_data_provider.initialize_rate_sources(
            [ConnectorPair(connector_name=config.connector_name,
    ...
    def should_effectivize_executor(self, executor_info, current_time: int) -> bool:
        ...
        effectivization_time = self.config.get_position_effectivization_time(
            trade_type)
    ...
    def create_actions_proposal(self) -> List[ExecutorAction]:
        ...
        buy_spreads, buy_amounts_quote = self.config.get_spreads_and_amounts_in_quote(
            TradeType.BUY)
        sell_spreads, sell_amounts_quote = self.config.get_spreads_and_amounts_in_quote(
            TradeType.SELL)
        ...
        for level_id in levels_to_execute:
            ...
            if trade_type == TradeType.BUY:
                spread_in_pct = Decimal(
                    buy_spreads[level]) * Decimal(self.processed_data["spread_multiplier"])
                amount_quote = Decimal(buy_amounts_quote[level])
            else:
                spread_in_pct = Decimal(
                    sell_spreads[level]) * Decimal(self.processed_data["spread_multiplier"])
                amount_quote = Decimal(sell_amounts_quote[level])

            # Calculate price and amount
            side_multiplier = Decimal(
                "-1") if trade_type == TradeType.BUY else Decimal("1")
            price = reference_price * \
                (Decimal("1") + side_multiplier * spread_in_pct)
            ...
            if amount == Decimal("0"):
                self.logger().warning(
                    f"The amount of the level {level_id} is 0. Skipping.")
                continue
    ...
    def get_levels_to_execute(self) -> List[str]:
        ...
        for analysis in all_levels_analysis:
            ...
            if (analysis["active_executors_not_trading"] or
                    ...
                    (analysis["open_order_last_update"] and
                     current_time - analysis["open_order_last_update"] < self.config.get_cooldown_time(trade_type)) or
                    (is_buy and analysis["min_price"] and
                     analysis["min_price"] * (Decimal("1") - self.config.min_buy_price_distance_pct) < current_price) or
                    (not is_buy and analysis["max_price"] and
                     analysis["max_price"] * (Decimal("1") + self.config.min_sell_price_distance_pct) > current_price)):
    ...
    async def update_processed_data(self):
        ...
        if position_held is not None:
            ...
            deviation = (target_position -
                         position_held.amount_quote) / target_position
            unrealized_pnl_pct = (
        ...
        if self.config.tick_mode:
            ...
            spread_multiplier = trading_rules.min_price_increment / reference_price
        else:
    ...
    def analyze_all_levels(self) -> List[Dict]:
        ...
        level_ids: Set[str] = {e.custom_info.get(
            "level_id") for e in self.executors_info if "level_id" in e.custom_info}
    ...
    def _analyze_by_level_id(self, level_id: str) -> Dict:
        ...
        filtered_executors = [e for e in self.executors_info if e.custom_info.get(
            "level_id") == level_id and e.is_active]

        active_not_trading = [
            e for e in filtered_executors if e.is_active and not e.is_trading]
        active_trading = [
            e for e in filtered_executors if e.is_active and e.is_trading]
        ...
        latest_open_order_update = max(
            open_order_last_updates) if open_order_last_updates else None

        prices = [e.config.entry_price for e in filtered_executors if hasattr(
            e.config, 'entry_price')]
    ...
    def to_format_status(self) -> List[str]:
        ...
        header_line = (
            ...
            f" | Position: {base_pct:.1%} ({self.config.min_base_pct:.1%}-{self.config.max_base_pct:.1%})"
            f" | PnL: {pnl_sign}{pnl:.2%}"
        ...
        status.append(
            f"│ {'Level':<12} │ {'Type':<6} │ {'State':<10} │ {'Price':<12} │"
            f" {'Amount':<12} │ {'Distance':<12} │ {'Age':<10} │"
        )
        status.append(
            f"├{'─' * 12}┼{'─' * 6}┼{'─' * 10}┼{'─' * 12}┼{'─' * 12}┼{'─' * 12}┼{'─' * 10}┤")

        # Analyze all levels and display each executor
        all_levels = self.analyze_all_levels()
        current_time = self.market_data_provider.time()

        for level_analysis in sorted(
            all_levels,
            key=lambda x: (not x["level_id"].startswith("buy"), x["level_id"]),
        ):
            level_id = level_analysis["level_id"]
            trade_type = "BUY" if level_id.startswith("buy") else "SELL"

            # Get all executors for this level
            level_executors = [e for e in self.executors_info if e.custom_info.get(
                "level_id") == level_id and e.is_active]
            ...
            for executor in level_executors:
                ...
                price = executor.config.entry_price if hasattr(
                    executor.config, 'entry_price') else Decimal("0")
                amount = executor.config.amount if hasattr(
                    executor.config, 'amount') else Decimal("0")

                # Calculate distance from current price
                if price > 0:
                    distance_pct = ((price - current_price) /
                                    current_price) * 100
                ...
                status.append(
                    f"│ {level_id:<12} │ {trade_type:<6} │ {state:<10} │ {price:<12.2f} │"
                    f" {amount:<12.4f} │ {distance_str:<12} │ {age_str:<10} │"
                )
