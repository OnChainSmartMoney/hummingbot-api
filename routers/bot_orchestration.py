...
@router.post("/{bot_name}/commands/custom_command")
async def send_bot_custom_command(
    bot_name: str,
    request: CustomCommandRequest,
    bots_manager: BotsOrchestrator = Depends(get_bots_orchestrator),
):
    """
    Send a custom command to a specific bot via MQTT.

    This is a generic endpoint that delegates to BotsOrchestrator.send_custom_command().
    'command' must be supported by the bot's MQTT command handler.
    """
    params = request.payload or {}
    result = await bots_manager.send_custom_command(
        bot_name=bot_name,
        params=params,
    )
    ...
    if not result.get("success"):
        ...
        # немного удобства: 404 если бота реально нет
...
def get_mqtt_status(bots_manager: BotsOrchestrator = Depends(get_bots_orchestrator)):
    ...
    # Check client state
    client_state = (
        "connected" if bots_manager.mqtt_manager.is_connected else "disconnected"
    )
...
def get_bot_status(
    bot_name: str, bots_manager: BotsOrchestrator = Depends(get_bots_orchestrator)
):
    """
...
async def get_bot_history(
    ...
    response = await bots_manager.get_bot_history(
        bot_name, days=days, verbose=verbose, precision=precision, timeout=timeout
    )
...
async def get_bot_run_by_id(
    bot_run_id: int, db_manager: AsyncDatabaseManager = Depends(get_database_manager)
):
    """
    ...
    try:
        async with db_manager.get_session_context() as session:
            ...
            run_dict = {
                ...
                "deployed_at": bot_run.deployed_at.isoformat()
                if bot_run.deployed_at
                else None,
                "stopped_at": bot_run.stopped_at.isoformat()
                if bot_run.stopped_at
                else None,
...
async def _background_stop_and_archive(
    ...
    try:
        ...
        if not stop_response or not stop_response.get("success", False):
            error_msg = (
                stop_response.get("error", "Unknown error")
                if stop_response
                else "No response from bot orchestrator"
            )
...
async def stop_and_archive_bot(
    ...
    try:
        ...
        if not bot_found:
            return {
                "status": "error",
                "message": f"Bot '{actual_bot_name}' not found in active bots. Active bots: {active_bots}."
                f"Cannot perform graceful shutdown.",
                "details": {
                    "input_name": bot_name,
        ...
        bot_name_for_orchestrator = (
            container_name if container_name in active_bots else actual_bot_name
        )
        ...
        return {
            ...
            "details": {
                ...
                "actual_bot_name": actual_bot_name,
                "container_name": container_name,
                "process": "The bot will be gracefully stopped, archived,"
...
async def deploy_v2_controllers(
    ...
    try:
        ...
        if deployment.max_global_drawdown_quote is not None:
            script_config_content["max_global_drawdown_quote"] = (
                deployment.max_global_drawdown_quote
            )
        if deployment.max_controller_drawdown_quote is not None:
            script_config_content["max_controller_drawdown_quote"] = (
                deployment.max_controller_drawdown_quote
            )
