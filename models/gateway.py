...
class GatewayConfig(BaseModel):
    ...
    image: str = Field(default="hummingbot/gateway:latest",
...
class GatewayStatus(BaseModel):
    ...
    container_id: Optional[str] = Field(
        default=None, description="Container ID if running")
    image: Optional[str] = Field(
        default=None, description="Image used for the container")
    created_at: Optional[str] = Field(
        default=None, description="Container creation timestamp")
    port: Optional[int] = Field(
        default=None, description="Port Gateway is running on")
...
class CreateWalletRequest(BaseModel):
    ...
    set_default: bool = Field(
        default=True, description="Set as default wallet for this chain")
...
class GatewayWalletCredential(BaseModel):
    ...
    network: Optional[str] = Field(
        default=None, description="Network to use (defaults to chain's default)")
...
class AddPoolRequest(BaseModel):
    ...
    fee_pct: Optional[float] = Field(
        default=None, description="Pool fee percentage (e.g., 0.25)")
...
class AddTokenRequest(BaseModel):
    ...
    name: Optional[str] = Field(
        default=None, description="Token name (defaults to symbol)")
...
class GatewayBalanceRequest(BaseModel):
    ...
    tokens: Optional[List[str]] = Field(
        default=None, description="List of token symbols to query (optional)")
